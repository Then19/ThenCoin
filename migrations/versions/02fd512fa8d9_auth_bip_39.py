...
Revision ID: 02fd512fa8d9
Revises: c6e90a4c105d
Create Date: 2023-01-23 09:06:12.927913

"""
from alembic import op

# revision identifiers, used by Alembic.
revision = '02fd512fa8d9'
down_revision = 'c6e90a4c105d'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS security.d_words (
        id SERIAL PRIMARY KEY,
        word VARCHAR(8) NOT NULL
    )""")

    conn.execute("""INSERT INTO security.d_words (word) VALUES """ + ', '.join(f"('{i}')" for i in words))

    conn.execute("""CREATE TABLE IF NOT EXISTS security.user_seeds (
        user_id UUID PRIMARY KEY,
        seed VARCHAR(64) NOT NULL,
        CONSTRAINT unic_seed UNIQUE (seed),
        CONSTRAINT fk_users FOREIGN KEY(user_id) REFERENCES users.user_card(user_id)
    )""")


def downgrade():
    conn = op.get_bind()
    conn.execute(""" DROP TABLE IF EXISTS security.d_words """)
    conn.execute(""" DROP TABLE IF EXISTS security.user_seeds """)
    conn.execute(""" DROP SCHEMA security """)
